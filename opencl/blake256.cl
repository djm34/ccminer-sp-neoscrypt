// (c) 2013 originally written by smolen
//     2014 ccminer adaptation by tpruvot

#define SPH_ROTR32(v,n) rotate((uint)(v),(uint)(32-(n)))

// precalc hash from first part of message
#define	h0 (const uint) in[0];
#define	h1 (const uint) in[1];
#define	h2 (const uint) in[2];
#define	h3 (const uint) in[3];
#define	h4 (const uint) in[4];
#define	h5 (const uint) in[5];
#define	h6 (const uint) in[6];
#define	h7 (const uint) in[7];

// last 12 bytes of original message
#define	in16 (const uint) in[8];
#define	in17 (const uint) in[9];
#define	in18 (const uint) in[10];

// to check : https://www.khronos.org/registry/cl/sdk/2.0/docs/man/xhtml/clEnqueueNDRangeKernel.html

__kernel /* __attribute__((reqd_work_group_size(WORKSIZE, 1, 1))) */
void search(
	volatile __global uint * restrict output,
	__global const uint * restrict in,
	const ulong target
)
{

    ...
    unsigned long  *hashes_done)
{
    ...
    int thpcoef = throughput;

    const uint32_t FirstNonce = pdata[29];
    static bool init[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };

    if (tp_coef[thr_id] > 0.0)
        thpcoef = (throughput * 100) / lround(tp_coef[thr_id] * 100.0);

    if (!init[thr_id])
    {
        cudaSetDevice(device_map[thr_id]);
        if (opt_debug)
            applog(LOG_DEBUG, "Throughput set to %d", thpcoef);

        cudaMalloc(&d_prod0[thr_id], 35 *sizeof(uint64_t) * thpcoef);
        cudaMalloc(&d_prod1[thr_id], 38 *sizeof(uint64_t) * thpcoef);
        cudaMalloc(&KeccakH[thr_id],  8 *sizeof(uint64_t) * thpcoef);
        cudaMalloc(&Sha512H[thr_id],  8 *sizeof(uint64_t) * thpcoef);

           m7_sha256_cpu_init(thr_id, thpcoef);
              sha512_cpu_init(thr_id, thpcoef);
        m7_keccak512_cpu_init(thr_id, thpcoef);
            haval256_cpu_init(thr_id, thpcoef);
            tiger192_cpu_init(thr_id, thpcoef);
        whirlpool512_cpu_init(thr_id, thpcoef, 0);
           ripemd160_cpu_init(thr_id, thpcoef);
         quark_check_cpu_init(thr_id, thpcoef);
               m7_bigmul_init(thr_id, thpcoef);
               mul_init();
    ...
    do {
        ...
        m7_keccak512_cpu_hash(thr_id, thpcoef, pdata[29], KeccakH[thr_id], order++);

        m7_sha512_cpu_hash_120(thr_id, thpcoef, pdata[29], Sha512H[thr_id], order++);

        cpu_mulT4(0, thpcoef, 8, 8, Sha512H[thr_id], KeccakH[thr_id], d_prod0[thr_id],order); //64
        MyStreamSynchronize(0,order++,thr_id);

        m7_whirlpool512_cpu_hash_120(thr_id, thpcoef, pdata[29], KeccakH[thr_id], order++);

        cpu_mulT4(0, thpcoef,8, 16, KeccakH[thr_id], d_prod0[thr_id], d_prod1[thr_id],order); //128
        MyStreamSynchronize(0,order++,thr_id);

        m7_sha256_cpu_hash_120(thr_id, thpcoef, pdata[29], KeccakH[thr_id], order++);
        cpu_mulT4(0, thpcoef, 4, 24, KeccakH[thr_id], d_prod1[thr_id], d_prod0[thr_id],order); //96
        MyStreamSynchronize(0,order++,thr_id);

        m7_haval256_cpu_hash_120(thr_id, thpcoef, pdata[29], KeccakH[thr_id], order++);
        cpu_mulT4(0, thpcoef, 4, 28, KeccakH[thr_id], d_prod0[thr_id], d_prod1[thr_id],order);  //112
        MyStreamSynchronize(0,order++,thr_id);

        m7_tiger192_cpu_hash_120(thr_id, thpcoef, pdata[29], KeccakH[thr_id], order++);
        m7_bigmul_unroll1_cpu(thr_id, thpcoef, KeccakH[thr_id], d_prod1[thr_id], d_prod0[thr_id],order);
        MyStreamSynchronize(0,order++,thr_id);

        m7_ripemd160_cpu_hash_120(thr_id, thpcoef, pdata[29], KeccakH[thr_id], order++);

        m7_bigmul_unroll2_cpu(thr_id, thpcoef, KeccakH[thr_id], d_prod0[thr_id], d_prod1[thr_id],order);
        MyStreamSynchronize(0,order++,thr_id);


        uint32_t foundNonce = m7_sha256_cpu_hash_300(thr_id, thpcoef, pdata[29], NULL, d_prod1[thr_id], order);
